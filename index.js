...
if (passwordProtected) {
    if (!hexo.config.admin.password_hash) {
        console.error('[Hexo MyAdmin]: config admin.password_hash is requred for authentication');
        passwordProtected = false;
    } else if (hexo.config.admin.password_hash.length <= 32) {
        throw new Error('[Hexo Myadmin]: the provided password_hash looks like an md5 hash -- hexo-myadmin has switched to use bcrypt; see the Readme for more info.')
    }

    if (!hexo.config.admin.secret) {
        console.error('[Hexo MyAdmin]: config admin.secret is requred for authentication');
        passwordProtected = false;
    }
