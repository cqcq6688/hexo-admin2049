...
module.exports = function(app, hexo) {
    ...
    function getSettings() {
        var path = hexo.base_dir + '_myadmin-config.yml'
        if (!fs.existsSync(path)) {
            hexo.log.d('admin config not found, creating one')
            fs.writeFile(hexo.base_dir + '_myadmin-config.yml', '')
            return {}
        } else {
    ...
    use('settings/set', function(req, res, next) {
        ...
        fs.writeFileSync(hexo.base_dir + '_myadmin-config.yml', yml.safeDump(settings))
        res.done({
